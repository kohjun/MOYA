...
## 0. 운영 전환 체크리스트 (Production Readiness)

현재 코드베이스는 **에뮬레이터/QA 친화 dev default** 가 일부 적용되어 있다. 운영 배포 전 아래 항목을 모두 통과시켜야 한다.

| # | 항목 | 위치 / 결정점 | 운영 기준 | 검증 |
|---|---|---|---|---|
| 1 | `allowGpsFallbackWithoutBle = false` | `backend/src/game/plugins/fantasy_wars_artifact/schema.js` (default `true`). host lobby config 또는 env 로 false override | BLE 미관측 결투 차단 | `resolveDuelConfig honors production override` 테스트 + 실 lobby 생성 시 false 강제 |
| 2 | 최소 총 인원 수 운영 기준 적용 | `backend/src/game/plugins/fantasy_wars_artifact/startValidation.js` 의 `DEFAULT_MIN_TOTAL_PLAYERS` (현재 3, dev 친화). 운영 권장 9 (3 팀 × 3) 이상 | 최소 인원 미달 lobby 거부 | 운영 룰 테스트 추가 후 통과 |
| 3 | 팀당 최소 인원 수 운영 기준 적용 | `startValidation.js` 의 `MIN_PLAYERS_PER_TEAM` (현재 1). 운영 권장 2-3 | 1-1-1 lobby 거부 | 동일 |
| 4 | Supabase/RAG 관련 env 실제 배포 환경에 주입 | `backend/.env` 의 `SUPABASE_URL`, `SUPABASE_SERVICE_KEY`, `GEMINI_API_KEY`. `ragRetriever.js` 는 lazy init 이라 미설정 시 silent 무동작 | RAG 동작 확인 | 실 호출 시 `[ragRetriever] error` 로그 미발생 |
| 5 | Council 30 초 제한이 실제 플레이에 적절한지 확인 | `backend/src/game/duel/DuelService.js` 의 `GAME_TIMEOUT_MS = 30_000`. council_bidding 은 chest reveal + 3 라운드 입찰로 빠듯할 수 있음 | 실 플레이어 데이터로 timeout 율 < 5% | 실 lobby 데이터 수집 후 평가 |
| 6 | BLE/GPS proximity 정책 운영 기준으로 검증 | `FantasyWarsBlePresenceService` (Flutter) ↔ `ProximityEvidence.js` (Backend) 의 RSSI 임계 / 관측 윈도우 / freshness 가 짝이 맞는지 | 실 BLE 페어 인정 ≥ 95% | 두 실기기로 페어 검증 시나리오 통과 |
| 7 | dev-only fallback 이 운영에서 켜져 있지 않은지 확인 | grep `[DEV DEFAULT]` (backend) 로 모든 임시 default 점검. host config / env 로 운영 값이 override 되었는지 lobby 생성 직후 로그 확인 | 모든 `[DEV DEFAULT]` 가 운영 값으로 덮였음 | 배포 직후 첫 lobby 의 publicState 검증 |

> 신규 dev default 를 추가할 때는 반드시 `[DEV DEFAULT] ...` 주석을 코드에 남기고 본 표에 한 줄을 추가한다.

...
class _PlayableAreaPainterScreenState
  ...
  Future<void> _refreshOverlays() async {
    ...
    if (_vertices.length >= 3) {
      // flutter_naver_map은 NPolygonOverlay가 닫힌 도형임을 요구한다.
      // (coords.first == coords.last assertion)
      final coords = List<NLatLng>.from(_vertices);
  ...
  Future<void> _save() async {
    ...
    try {
      // 폴리곤을 닫아서 저장 — flutter_naver_map NPolygonOverlay는
      // coords.first == coords.last assertion을 요구한다.
      final closedVertices = List<NLatLng>.from(_vertices);
      if (closedVertices.isNotEmpty &&
          closedVertices.first != closedVertices.last) {
        closedVertices.add(closedVertices.first);
      }
      final points = closedVertices
      ...
      // 서버 저장 성공 즉시 GameProvider 상태에도 반영 →
      // 게임 메인 화면에서 ref.listen으로 폴리곤을 즉시 렌더링하게 된다.
      ref
          .read(gameProvider(widget.sessionId).notifier)
          .setPlayableArea(saved);
      ...
      navigator.pop(saved); // 저장된 좌표를 호출자에게 반환
    } catch (e) {
      if (!mounted) return;
      messenger.showSnackBar(
